...
function feedApiRouter(io) {
    router.get("/", async (req, res) => {
        try {
            ...
            let studentDocID = (await user_service_1.Convert.getDocumentID_studentid(req.session["stdid"])).toString();
            logger_1.default.info(`(/feed): Converted to documentID from studentID=${req.session["stdid"] || '--studentID--'}`);
            let notes = await (0, post_service_1.getPosts)(studentDocID, { skip: skip, limit: count, seed: seed });

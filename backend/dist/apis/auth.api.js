...
function authApiRouter(io) {
    router.post("/signup", async (req, res) => {
        try {
            ...
            if (!(displayname && email && password)) {
                ...
                const response = await (0, auth_service_1.addUserProfile)(studentData);
                if (response.ok) {
    ...
    router.post("/login", async (req, res) => {
        try {
            ...
            if (email && password && email.length !== 0 && password.length !== 0) {
                ...
                let response = await (0, auth_service_1.getUserVarification)(email);
                if (response.ok) {
    ...
    router.get("/session", async (req, res) => {
        try {
            if (req.session && req.session["stdid"]) {
                const response = await (0, auth_service_1.getUserAuth)(req.session["stdid"]);
                if (response.ok) {
    ...
    router.post('/google', async (req, res) => {
        try {
            ...
            const existingUser = await (0, auth_service_1.getUserVarification)(email);
            if (existingUser.ok) {
            ...
            const response = await (0, auth_service_1.addUserProfile)(newUser);
            if (response.ok) {

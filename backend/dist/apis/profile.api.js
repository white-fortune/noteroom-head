...
function profileApiRouter(io) {
    router.get("/mutual-college", async (req, res) => {
        try {
            ...
            let profiles = await (0, user_service_1.getMutualCollegeStudents)(studentDocID, { count: count, skip: skip, countDoc });
            res.json(profiles);
        }
        catch (error) {
            res.json([]);
    ...
    router.get("/:username", async (req, res) => {
        try {
            if (req.params.username) {
                ...
                let profileStudentID = await user_service_1.Convert.getStudentID_username(username);
                let profile = await (0, user_service_1.getProfile)(username);
            }
            else {
    ...
    router.post("/change", async (req, res) => {
        try {
            ...
            const response = await (0, user_service_1.updateProfileFields)(studentID, updates);
            if (response.ok) {

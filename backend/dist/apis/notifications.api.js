...
function notificationApiRouter(io) {
    router.get("/", async (req, res) => {
        try {
            ...
            let response = await (0, notification_service_1.getNotifications)(studentID);
            if (response.ok) {
    ...
    router.delete("/", async (req, res) => {
        try {
            let studentID = req.session["stdid"];
            let deletedResult = await (0, notification_service_1.deleteAllNoti)(studentID);
    ...
    router.get("/:notificationID/read", async (req, res) => {
        try {
            ...
            const response = await (0, notification_service_1.readNoti)(notiID);

...
function requestsApiRouter(io) {
    router.post('/:requestID/accept', async (req, res) => {
        try {
            ...
            const response = await (0, requestService_1.getRequest)(requestID);
            if (response.ok) {
                ...
                await (0, notificationService_1.NotificationSender)(io, {
                    ...
                    event: notificationService_1.NotificationEvent.NOTIF_REQUEST_ACCEPT,
                    isInteraction: true,
                    fromUserSudentDocID: receiverDocID
                });
                const deleteResponse = await (0, requestService_1.deleteRequest)(requestID);
        ...
        try {
            ...
            if (response.ok) {
                ...
                await (0, notificationService_1.NotificationSender)(io, {
                    ...
                    event: notificationService_1.NotificationEvent.NOTIF_REQUEST_DECLINE,
                    isInteraction: true,
                    fromUserSudentDocID: receiverDocID
                });
                const deleteResponse = await (0, requestService_1.deleteRequest)(requestID);
    ...
    router.post('/send', async (req, res) => {
        try {
            ...
            let senderDocID = await userService_1.Convert.getDocumentID_studentid(senderStudentID);
            let receiverUsername = req.body.receiverUsername;
            let receiverDocID = await userService_1.Convert.getDocumentID_username(receiverUsername);
            if (senderDocID !== receiverDocID) {
                const requestData = {
                    senderDocID, receiverDocID,
                    message: req.body.message
                };
                const response = await (0, requestService_1.addRequest)(requestData);
                if (response.ok) {
                    ...
                    await (0, notificationService_1.NotificationSender)(io, {
                        ...
                        event: notificationService_1.NotificationEvent.NOTIF_REQUEST,
                        isInteraction: true,

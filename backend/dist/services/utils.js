...
async function processBulkCompressUpload(fileObjects, postID) {
    try {
        ...
        let uploadedFiles = await Promise.all(compressedFiles.map(compressedFile => (0, firebase_service_1.upload)(compressedFile, `posts/${postID}/contents/${compressedFile["fileName"]}`)));
...
async function processBuikPDFUpload(fileObjects, postID) {
    try {
        ...
        await Promise.all(fileObjects.map(async (file) => {
            const publicUrl = await (0, firebase_service_1.upload)(file, `posts/${postID}/contents/${file["fileName"]}`);
            if (publicUrl) {

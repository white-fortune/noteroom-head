...
async function getNotifications(ownerStudentID) {
    try {
        let notifications = await notifications_1.Notifs.aggregate([
...
async function deleteAllNoti(ownerStudentID) {
    try {
        let result = await notifications_1.Notifs.deleteMany({ ownerStudentID: ownerStudentID });
...
async function readNoti(notiID) {
    try {
        await notifications_1.Notifs.updateOne({ _id: notiID }, { $set: { isRead: true } });
        return { ok: true };
    }
    catch (error) {
        return { ok: false };
...
async function deleteNoti(notiID) {
    await notifications_1.Notifs.deleteOne({ _id: notiID });
}
async function addNoti(notiData) {
    let data = await notifications_1.Notifs.create(notiData);
    return data;
}
async function addInteractionNoti(notiData) {
    let data = await notifications_1.InteractionNotifs.create(notiData);

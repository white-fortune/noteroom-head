...
async function getUserAuth(studentID_) {
    try {
        const authData = await students_1.default.findOne({ studentID: studentID_ }, { studentID: 1, username: 1, _id: 0 });
...
async function getUserVarification(email) {
    try {
        let student = await students_1.default.findOne({ email: email });
        if (student) {
            return { ok: true, data: {
...
async function addUserProfile(user) {
    try {
        const userData = await students_1.default.create(user);
